...
- обычный batch из 1–3 задач явно определён как ориентир планирования, а не
  ограничение или условие завершения фазы;
- `PHASE COMPLETE` теперь во всех ключевых инструкциях зависит только от
  проверенных acceptance criteria;
- при недоступности staging credentials, интеграций или иной внешней
  инфраструктуры агент обязан назвать конкретный блокер и затронутый критерий,
  а не ссылаться на размер batch;
- `tools/audit.py` проверяет это правило в канонических инструкциях и основных
  prompts, предотвращая регрессию.
